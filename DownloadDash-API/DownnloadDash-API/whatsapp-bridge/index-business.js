...
const SESSION_DIR = path.join(__dirname, '../auth/whatsapp-business');
const DOWNLOADS_DIR = path.join(__dirname, '../downloads/whatsapp-business');
...
function listDownloadedStatuses() {
  ...
  const files = fs.readdirSync(DOWNLOADS_DIR, { withFileTypes: true })
    .filter((d) => d.isFile())
    .map((d) => d.name)
    .filter((name) => /^whatsapp_business_status_\d+\.(jpg|jpeg|png|webp|mp4)$/i.test(name));
...
async function connectToWhatsApp() {
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (qr) {
      qrCode = qr;
      console.log('WhatsApp Business QR Code updated, scan with WhatsApp Business');
    ...
    if (connection === 'close') {
      ...
      console.log('WhatsApp Business connection closed, reconnecting:', shouldReconnect);
...
async function handleStatusMessage(msg) {
  try {
    ...
    if (msg.message?.imageMessage || msg.message?.videoMessage) {
      ...
      const filename = `whatsapp_business_status_${Date.now()}.${mediaType === 'image' ? 'jpg' : 'mp4'}`;
      fs.writeFileSync(path.join(DOWNLOADS_DIR, filename), buffer);
      console.log('WhatsApp Business status saved:', filename);
    }
  } catch (e) {
    console.error('Failed to handle WhatsApp Business status:', e?.message || e);
  }
}
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/qr') {
      return sendJson(res, 200, {
        qr: qrCode,
        message: qrCode ? 'Scan with WhatsApp Business' : 'Already connected or no QR available',
      });
    }
...
const PORT = Number(process.env.WA_BUSINESS_BRIDGE_PORT || 3005);
server.listen(PORT, () => {
  console.log(`WhatsApp Business bridge running on port ${PORT}`);
  connectToWhatsApp().catch((e) => console.error('WhatsApp Business connect failed:', e));
});
