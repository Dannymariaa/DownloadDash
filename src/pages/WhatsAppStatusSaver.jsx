...
const BRIDGE_BASE_URL = import.meta.env.VITE_WA_BRIDGE_URL || 'http://127.0.0.1:3001';
...
export default function WhatsAppStatusSaver() {
  ...
  const loadStatuses = async () => {
    ...
    try {
      ...
      setStatusLoadError(
        'No statuses found. Make sure the WhatsApp bridge is running on port 3001 and you are connected.'
      );
    } finally {
  ...
  const handleSaveSelected = async () => {
        ...
        for (const id of selectedItems) {
          ...
          if (item) {
            await downloadDash.entities.SavedContent.create({
              ...
              platform: 'whatsapp',
              content_type: item.type === 'video' ? 'status' : 'image',
              content_url: item.url,
              thumbnail_url: item.thumbnail || undefined,
              title: `WhatsApp Status - ${item.timestamp}`
            });
          }
        }
      } catch {
    ...
    try {
      ...
      for (const id of selectedItems) {
        ...
        const filename = item.filename || `whatsapp_${item.type}_${Date.now()}`;
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        >
          <motion.div 
            className="w-24 h-24 mx-auto rounded-3xl bg-gradient-to-br from-green-600 to-green-400 flex items-center justify-center shadow-2xl mb-6"
          ...
          <p className="text-gray-400 text-lg mb-2">
            Save status videos and images from your contacts
          </p>
          
          <div className="flex flex-wrap justify-center gap-2 mb-6">
            {['Status Videos', 'Status Images', 'No Login Required', 'Save to Gallery'].map((tag, idx) => (
              <span 
                key={idx}
                className="px-4 py-1 rounded-full bg-green-500/20 text-green-300 border border-green-500/30 text-sm"
              >
                {tag}
              </span>
            ))}
        ...
        <motion.div
          ...
          className="bg-gradient-to-br from-green-900/20 to-black rounded-2xl p-6 border border-green-500/20 mb-8"
        >
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {[
              { step: '1', icon: <Smartphone className="h-6 w-6" />, text: 'View status in WhatsApp app' },
              { step: '2', icon: <Folder className="h-6 w-6" />, text: 'Status saves locally on your device' },
        ...
        >
          ...
          {!statusLoadError && !isStatusLoading && (activeTab === 'images' ? statusItems.images.length === 0 : statusItems.videos.length === 0) && (
            <div className="mt-6 text-center text-gray-500 text-sm">
              No {activeTab} found yet. View a status in WhatsApp, keep the bridge connected, then refresh.
            </div>
          )}
          ...
          <AnimatePresence>
            {selectedItems.length > 0 && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: 20 }}
                className="mt-6 p-4 bg-green-900/30 rounded-xl border border-green-500/30 flex items-center justify-between"
              >
        ...
        >
          ...
          <div className="text-sm text-gray-400">
            ...
            <p>You must first view the status in WhatsApp for it to appear here. WhatsApp temporarily saves viewed statuses on your device, which this app can access to help you save them permanently.</p>
          </div>
        </motion.div>
